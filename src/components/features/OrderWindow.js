...
const MyOrderWindow = ({ className }) => {
  ...
  //redux
  const orderSearchResult = useSelector(
    (state) => state.orderEnquiryReducer.orderSearchResult
  );
  const showOrderWindow = useSelector(
    (state) => state.orderEnquiryReducer.showOrderWindow
  );
