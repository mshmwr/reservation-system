...
const MyDropdowns = ({ langs, language, switchLanguageClickHandler, className, listItemHeight, listItemWidth }) => {
    return (
        <div className={`${className} menu__language`}>
            {Object.keys(langs).filter((key => language === langs[key].multiLang)).map((key => <div key={key} className="menu__language__current">{langs[key].label}
                ...
                <ul className="menu__language__list">
                    {Object.keys(langs).map((key) => (
                        <li
                            key={langs[key].label}
                            className={`menu__language__list__item ${language === langs[key].multiLang
